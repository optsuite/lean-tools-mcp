...
section FileWorker
  ...
  /-- Communication handle for a FileWorker: either a child process or an in-process task. -/
  inductive WorkerHandle where
    /-- Worker runs as a child process (original behavior). -/
    | process   (proc : Process.Child workerCfg)
    /-- Worker runs as an in-process task communicating via `Std.Channel`.
        `inCh`  = Watchdog → Worker (worker reads from it).
        `outCh` = Worker → Watchdog (worker writes to it).
        `task`  = the spawned worker task. -/
    | inProcess (inCh : Std.Channel String) (outCh : Std.Channel String)
  ...
  structure FileWorker where
    ...
    stdinStream  : FS.Stream
    stdoutStream : FS.Stream
    commTask?    : Option (ServerTask WorkerEvent)
    state        : Std.Mutex WorkerState

  namespace FileWorker

  def stdin (fw : FileWorker) : FS.Stream := fw.stdinStream

  def stdout (fw : FileWorker) : FS.Stream := fw.stdoutStream

  /-- Map an in-process task result to an exit code.
      Exit code 2 signals imports-changed (triggers worker restart). -/
  private def taskResultToExitCode (result : Except IO.Error Unit) : UInt32 :=
    match result with
    | .ok ()  => 0
    | .error e =>
      if e.toString == FileWorker.importsChangedMsg then 2
      else 1
  ...
  def waitForProc (fw : FileWorker) : IO UInt32 :=
    fw.exitCode.atomically do
      match ← get with
      | none =>
        let exitCode ← match fw.handle with
          | .process proc => proc.wait
          | .inProcess _ _ task => pure (taskResultToExitCode task.get)
        set <| some exitCode
        return exitCode
      | some exitCode =>
        return exitCode
  ...
  def killProcAndWait (fw : FileWorker) : IO UInt32 :=
    fw.exitCode.atomically do
      match ← get with
      | none =>
        match fw.handle with
        | .process proc =>
          proc.kill
          let exitCode ← proc.wait
          set <| some exitCode
          return exitCode
        | .inProcess inCh _ task =>
          -- Signal worker to stop by sending EOF, then wait for task
          try inCh.sync.send "" catch _ => pure ()
          let exitCode := taskResultToExitCode task.get
          set <| some exitCode
          return exitCode
      | some exitCode =>
        return exitCode

  /-- Best-effort kill: sends SIGKILL for process workers, no-op for in-process workers. -/
  def tryKill (fw : FileWorker) : IO Unit :=
    match fw.handle with
    | .process proc    => proc.kill
    | .inProcess .. => pure ()
...
section ServerM
  ...
  structure ServerContext where
    ...
    /-- Parsed Lean options from command line (-D, -M, -T flags).
        Used for in-process FileWorker tasks. -/
    leanOpts          : Options
    /-- If true, FileWorkers run as in-process tasks instead of child processes.
        Controlled by `LEAN_WORKER_INPROCESS=1` environment variable. -/
    useInProcessWorkers : Bool
  ...
  /-- Shared post-creation setup: register worker, start message forwarding,
      send initialize + didOpen, replay queued requests. -/
  private def finishFileWorkerSetup (fw : FileWorker) (m : DocumentMeta) : ServerM Unit := do
    let st ← read
    ...
    let fw := { fw with doc := { m with dependencyBuildMode := updatedDependencyBuildMode} }
  ...
  /-- Start a FileWorker as a child process (original behavior). -/
  private def startFileWorkerProcess (m : DocumentMeta) : ServerM Unit := do
    let st ← read
    ...
    let fw : FileWorker := {
      doc := m
      handle := .process workerProc
      exitCode
      stdinStream  := FS.Stream.ofHandle workerProc.stdin
      stdoutStream := FS.Stream.ofHandle workerProc.stdout
      commTask? := none
      state := ← Std.Mutex.new WorkerState.running
    }
    return (← finishFileWorkerSetup fw m)

  /-- Start a FileWorker as an in-process task using Channel ↔ Stream adapters. -/
  private def startFileWorkerInProcess (m : DocumentMeta) : ServerM Unit := do
    let st ← read
    let inCh ← Std.Channel.new    -- Watchdog → Worker
    let outCh ← Std.Channel.new   -- Worker → Watchdog

    let workerIn ← Lean.Server.channelInputStream inCh
    let workerOut ← Lean.Server.channelOutputStream outCh
    let workerErr ← IO.getStderr
    let workerErr := workerErr.withPrefix s!"[in-process:{m.uri}] "

    let workerOpts := st.leanOpts
    let task ← IO.asTask (prio := .dedicated) do
      try
        FileWorker.initAndRunWorker workerIn workerOut workerErr workerOpts
      finally
        -- Send EOF sentinel so forwardMessages detects termination
        outCh.sync.send ""

    let wdStdin ← Lean.Server.channelOutputStream inCh
    let wdStdout ← Lean.Server.channelInputStream outCh

    let exitCode ← Std.Mutex.new none
    let fw : FileWorker := {
      doc := m
      handle := .inProcess inCh outCh task
      exitCode
      stdinStream  := wdStdin
      stdoutStream := wdStdout
      commTask? := none
      state := ← Std.Mutex.new WorkerState.running
    }
    return (← finishFileWorkerSetup fw m)

  /-- Start a FileWorker using the configured mode (process or in-process). -/
  def startFileWorker (m : DocumentMeta) : ServerM Unit := do
    let st ← read
    writeMessage <| mkFileProgressAtPosNotification m 0
    if st.useInProcessWorkers then
      startFileWorkerInProcess m
    else
      startFileWorkerProcess m
...
section NotificationHandling
  def handleDidOpen (p : LeanDidOpenTextDocumentParams) : ServerM Unit := do
    ...
    -- PATCH: Support dynamic/temporary files that are not in the module graph
    -- Try to get module name from URI, but fall back to a synthetic name if it fails
    let mod ← try
      moduleFromDocumentUri doc.uri
    catch _ =>
      -- For temporary files (e.g., .lake/lean_tools_mcp/*.lean), create a synthetic module name
      -- This allows LSP to check them even though they're not in the project's module graph
...
def initAndRunWatchdog (args : List String) (i o : FS.Stream) (opts : Options := {}) : IO Unit := do
  ...
  let useInProcess := match (← IO.getEnv "LEAN_WORKER_INPROCESS") with
  ...
  ReaderT.run initAndRunWatchdogAux {
    ...
    workerPath
    leanOpts       := opts
    useInProcessWorkers := useInProcess
    referenceData
    serverRequestData
    importData
    requestData
...
def watchdogMain (args : List String) (opts : Options := {}) : IO UInt32 := do
  let i ← IO.getStdin
  let o ← IO.getStdout
  let e ← IO.getStderr
  try
    initAndRunWatchdog args i o opts
