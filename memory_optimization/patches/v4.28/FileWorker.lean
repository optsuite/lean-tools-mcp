...
public import Lean.Server.ChannelStream
...
/-- Sentinel error message used by in-process FileWorkers to signal that imports
    have changed and the worker should be restarted.  The Watchdog checks for this
    specific string to map it to exit code 2 (`.importsChanged`). -/
def importsChangedMsg : String := "lean-server:imports-changed"
...
`importsLoadedRef` is per-worker: each worker creates its own ref so that
in-process workers don't interfere with each other's import loading.
-/
def setupImports
    (importsLoadedRef : IO.Ref Bool)
    (doc         : DocumentMeta)
    (cmdlineOpts : Options)
    (chanOut     : Std.Channel OutputMessage)
  ...
  if importsAlreadyLoaded then
    ...
    unless (← IO.checkCanceled) do
      throw (IO.userError importsChangedMsg)
...
section Initialization
  def initializeWorker (doc : DocumentMeta) (o e : FS.Stream) (initParams : InitializeParams) (opts : Options)
    ...
    let importsLoadedRef ← IO.mkRef false
    let processor := Language.Lean.process (setupImports importsLoadedRef doc opts chanOut)
  ...
  partial def mainLoop : WorkerM Unit := do
    ...
    | Message.request id method (some params) =>
      try
        ...
        let ctx ← read
        ctx.hLog.putStrLn s!"Request handler error (recovered): {err}"
        ctx.chanOut.sync.send <| .ofMsg <|
          (RequestError.internalError (toString err)).toLspResponseError id
      mainLoop
    ...
    | Message.notification method (some params) =>
      try
        ...
        let ctx ← read
        ctx.hLog.putStrLn s!"Notification handler error (recovered): {err}"
      mainLoop
    ...
    | _ =>
      let ctx ← read
      ctx.hLog.putStrLn s!"Got invalid JSON-RPC message, ignoring"
      mainLoop
end MainLoop
...
def initAndRunWorker (i o e : FS.Stream) (opts : Options) : IO Unit := do
  ...
  StateRefT'.run' (s := st) <| ReaderT.run (r := ctx) do
    try
      ...
      ctx.hLog.putStrLn s!"mainLoop terminated with error: {err}"
where
